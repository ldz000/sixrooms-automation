...
import sixrooms.base.GetConfigUtil;
...
public class TestPopUp {
    ...
    public static String shiLiu = GetConfigUtil.getTestProperty("element", "shiLiu");
    public static String xiuChang = GetConfigUtil.getTestProperty("element", "xiuChang");
    public static String lianYun = GetConfigUtil.getTestProperty("element", "lianYun");
    ...
    public void setUp() {
        ...
        int appPackage1 = sc.nextInt();
        if (appPackage1 == 1) {
            appPackage = shiLiu;
        } else if (appPackage1 == 2) {
            appPackage = xiuChang;
        } else if (appPackage1 == 3) {
            appPackage = lianYun;
        }
        sc.close();
        try {
    ...
    public void initialApi() {
        ...
        setUp();
        if (!waitForElement(By.id(appPackage + ":id/web_view_h5")))
            throw new RuntimeException("首页弹窗没弹出来");
        driver.pressKeyCode(AndroidKeyCode.BACK);
        if (!waitForElement(By.id(appPackage + ":id/view_hot_banner")))
            throw new RuntimeException("首页创可贴没显示出来");
        Point point = driver.findElementById(appPackage + ":id/view_hot_banner").getLocation();
        tAction.press(PointOption.point(point.x + 10, point.y + 10)).release().perform();
        if (!waitForElement(By.id(appPackage + ":id/common_web_view")))
        ...
        driver.findElementById(appPackage + ":id/iv_title_serach_cancle").click();
        waitForElement(By.id(appPackage + ":id/icon"));
        driver.findElementById(appPackage + ":id/icon").click();
        // targetClick(930,1010);
        if (waitForElement(By.id(appPackage + ":id/tv_living"))) {
            driver.findElementById(appPackage + ":id/tv_living").click();
        }
        targetClick(533, 1183);
        if (!waitForElement(By.id(appPackage + ":id/web_view_h5")))
            throw new RuntimeException("视频房主动弹窗没弹出来");
        driver.pressKeyCode(AndroidKeyCode.BACK);
        if (!waitForElement(By.id(appPackage + ":id/first_banner")))
            throw new RuntimeException("1号创可贴没显示出来");
        Point point1 = driver.findElementById(appPackage + ":id/first_banner").getLocation();
        tAction.press(PointOption.point(point1.x, point1.y)).release().perform();
        if (!waitForElement(By.id(appPackage + ":id/common_web_view")))
            throw new RuntimeException("1号创可贴点击没响应");
        driver.pressKeyCode(AndroidKeyCode.BACK);
        if (!waitForElement(By.id(appPackage + ":id/left_top_banner")))
            throw new RuntimeException("3号创可贴没显示");
        Point point3 = driver.findElementById(appPackage + ":id/left_top_banner").getLocation();
        tAction.press(PointOption.point(point3.x + 10, point3.y + 10)).release().perform();
        if (!waitForElement(By.id(appPackage + ":id/common_web_view")))
            throw new RuntimeException("3号创可贴点击没响应");
        driver.pressKeyCode(AndroidKeyCode.BACK);
        //4号创可贴
        if (!waitForElement(By.id(appPackage + ":id/right_top_banner")))
            throw new RuntimeException("4号创可贴没显示");
        Point point4 = driver.findElementById(appPackage + ":id/right_top_banner").getLocation();
        tAction.press(PointOption.point(point4.x + 10, point4.y + 10)).release().perform();
        if (!waitForElement(By.id(appPackage + ":id/common_web_view")))
            throw new RuntimeException("4号创可贴点击没响应");
        driver.pressKeyCode(AndroidKeyCode.BACK);
        //5号创可贴
        if (!waitForElement(By.id(appPackage + ":id/bottom_center_2_banner")))
            throw new RuntimeException("5号创可贴没显示");
        Point point5 = driver.findElementById(appPackage + ":id/bottom_center_2_banner").getLocation();
        tAction.press(PointOption.point(point5.x + 10, point5.y + 10)).release().perform();
        if (!waitForElement(By.id(appPackage + ":id/common_web_view")))
            throw new RuntimeException("5号创可贴点击没响应");
        driver.pressKeyCode(AndroidKeyCode.BACK);
        //6号创可贴
        if (!waitForElement(By.id(appPackage + ":id/bottom_center_banner")))
            throw new RuntimeException("6号创可贴没显示");
        Point point6 = driver.findElementById(appPackage + ":id/bottom_center_banner").getLocation();
        tAction.press(PointOption.point(point6.x + 10, point6.y + 10)).release().perform();
        if (!waitForElement(By.id(appPackage + ":id/common_web_view")))
            throw new RuntimeException("6号创可贴点击没响应");
        driver.pressKeyCode(AndroidKeyCode.BACK);
        driver.findElementById(appPackage + ":id/iv_close_room").click();
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        driver.findElementById(appPackage + ":id/iv_back").click();
        waitForElement(By.id(appPackage + ":id/ivClearKeyWord"));
        driver.findElement(By.id(appPackage + ":id/ivClearKeyWord")).click();
        try {
    ...
    public void php320_Test() {
        ...
        setUp();
        waitForElement(By.id(appPackage + ":id/web_view_h5"));
        driver.pressKeyCode(AndroidKeyCode.BACK);
        targetClick(65, 173);//点击查找按钮
        try {
            Runtime.getRuntime().exec("adb shell input text 232740372");
        ...
        if (!waitForElement(By.id(appPackage + ":id/second_banner")))
            throw new RuntimeException("320socket视频房倒数第二创可贴没显示出来");
        Point point2 = driver.findElementById(appPackage + ":id/second_banner").getLocation();
        ...
        Point point3 = driver.findElementById(appPackage + ":id/second_banner").getLocation();
        tAction.press(PointOption.point(point3.x + 10, point3.y + 10)).release().perform();
        if (!waitForElement(By.id(appPackage + ":id/common_web_view")))
            throw new RuntimeException("2号创可贴点击没响应");
        driver.pressKeyCode(AndroidKeyCode.BACK);
    ...
    public void php322_Test() {
        ...
        setUp();
        waitForElement(By.id(appPackage + ":id/mainBottomBarButtonImage"));
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        ...
        driver.pressKeyCode(AndroidKeyCode.BACK);
        driver.findElementById(appPackage + ":id/iv_close_room").click();
        waitForElement(By.id(appPackage + ":id/radioroom_exit_linearlayout"));
        driver.findElementById(appPackage + ":id/radioroom_exit_linearlayout").click();
        try {
            http.doGet("http://v.6.cn/api/doTestPop.php?act=update&eventname=test3&status=0");
            http.doGet(
